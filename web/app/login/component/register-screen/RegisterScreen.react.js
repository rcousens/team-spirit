...
var RegisterScreen = React.createClass({
    ...
    componentDidMount: function() {
        $(this.refs.password.getDOMNode()).complexify({strengthScaleFactor: 0.33}, function(valid, complexity) {
    ...
    render: function () {
        ...
        if (this.state.password.length >= 8 && this.state.password === this.state.password_confirm && parseInt(complexitystyle.width) === 100) {
            var equalPassword = true;
        } else {
            equalPassword = false;
        }

        if (this.props.email && this.props.email.length > 0 && this.refs.email.getDOMNode().checkValidity()) {
            var validEmail = true;
        } else {
            validEmail = false;
        }
        ...
        var emailErrorClass = validEmail ? 'form-group has-success' : 'form-group';
        var emailErrorLabel = '';
                ...
                <div className="jumbotron">
                    ...
                    <form action="#">
                        <div className={emailErrorClass}>
                            ...
                            <input type="email" ref="email" value={this.props.email} onChange={this.handleEmailChange} className="form-control" id="email" name="fos_user_registration_form[email]" placeholder="Email Address" />
                            {emailErrorLabel}
                        </div>
